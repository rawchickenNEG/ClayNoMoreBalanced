...
public class BlazeLever extends Item {

    public BlazeLever(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            clayball.setItem(ItemRegistry.BLAZE_RIDER_CLAY_BALL.get().getDefaultInstance());
            clayball.setType(1);
            clayball.setLever(true);
            clayball.setNoGravity(true);
            clayball.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 0.8F, 0F);
