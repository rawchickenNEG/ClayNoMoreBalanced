...
import io.github.rawchickenneg.cnmd.common.entity.ThrownBaseClayBall;
...
public class BaseClayBall extends Item {
    public BaseClayBall(Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ThrownBaseClayBall clayball = new ThrownBaseClayBall(level, player);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn)
    {
        ...
        String damage = String.valueOf(Config.CONFIG.BASE.get());
        tooltip.add(new TranslatableComponent("item.clay_no_more_balanced.stone_clay_ball.tip").withStyle(ChatFormatting.YELLOW));
        tooltip.add(new TranslatableComponent("item.clay_no_more_balanced.base_clay_ball.tip").withStyle(ChatFormatting.LIGHT_PURPLE));
        tooltip.add(new TranslatableComponent("item.clay_no_more_balanced.stone_clay_ball.tip2").withStyle(ChatFormatting.RED));
